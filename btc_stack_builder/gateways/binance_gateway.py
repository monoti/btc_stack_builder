...
class BinanceGateway(ExchangeGateway):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise GatewayError(f"Binance gateway initialization failed: {str(e)}")
    ...
    async def get_positions(self) -> List[Position]:
        ...
        try:
            ...
            futures_positions = await self._execute_with_retry('fetch_positions', self.futures_client)
            ...
            perp_positions = await self._execute_with_retry('fetch_positions', self.perpetual_client)
    ...
    async def get_funding_rate(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                'funding_time': timestamp_to_datetime(funding_info.get('nextFundingTime', 0) / 1000),
    ...
    async def get_futures_basis(self, spot_symbol: str, futures_symbol: str) -> Decimal:
        ...
        try:
            ...
            if not expiry_date:
                raise ValueError(f"Unable to parse expiry date from futures symbol: {futures_symbol}")
            ...
            logger.error(
                f"Error calculating futures basis",
    ...
    async def transfer_margin(self, amount: Decimal, asset: str, from_type: str, to_type: str) -> bool:
        """
        ...
        try:
            ...
            result = await self._execute_with_retry(
                ...
                self.spot_client,
                {
                    'type': 'MAIN_UMFUTURE' if (from_type_binance == 'SPOT' and to_type_binance == 'FUTURES') else 'UMFUTURE_MAIN',
                    'asset': asset,
                    'amount': float(amount),
                }
            )
    ...
    async def get_open_orders(self, symbol: Optional[str] = None) -> List[Order]:
        ...
        try:
            ...
            if symbol:
                ...
                futures_orders = await self._execute_with_retry('fetch_open_orders', self.futures_client)
                perp_orders = await self._execute_with_retry('fetch_open_orders', self.perpetual_client)

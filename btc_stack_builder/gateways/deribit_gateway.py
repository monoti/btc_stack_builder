...
class DeribitGateway(ExchangeGateway):
        ...
        try:
            ...
            logger.info(
                ...
                options_instruments=len(
                    [i for i in self.client.markets.values() if i.get("type") == "option"]
                ),
                futures_instruments=len(
                    [i for i in self.client.markets.values() if i.get("type") == "future"]
                ),
                spot_instruments=len(
                    [i for i in self.client.markets.values() if i.get("type") == "spot"]
                ),
            )
            ...
            raise AuthenticationError(f"Deribit authentication failed: {str(e)}") from e
        except ccxt.NetworkError as e:
            logger.error("Deribit connection failed", exc_info=True)
            raise GatewayConnectionError(f"Deribit connection failed: {str(e)}") from e
        except Exception as e:
            logger.error("Deribit gateway initialization failed", exc_info=True)
            raise GatewayError(f"Deribit gateway initialization failed: {str(e)}") from e
    ...
    async def _handle_ccxt_error(self, e: Exception) -> None:
        ...
        if isinstance(e, ccxt.AuthenticationError):
            raise AuthenticationError(f"Deribit authentication error: {str(e)}") from e
    ...
    def _parse_option_symbol(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Error parsing option symbol: {symbol}") from e
    ...
    async def get_positions(self) -> list[Position]:
        ...
        try:
            ...
            positions = await self._execute_with_retry(
                "privateGetGetPositions", {"currency": "BTC"}
            )
    ...
    async def get_margin_status(self) -> MarginStatus:
        ...
        try:
            ...
            account_summary = await self._execute_with_retry(
                "privateGetGetAccountSummary", {"currency": "BTC"}
            )
    ...
    async def get_option_chain(
        self, underlying: str = "BTC", expiry_range_days: tuple[int, int] = (0, 180)
        ...
        try:
            ...
            for instrument in instruments.get("result", []):
                ...
                option_data = {
                    ...
                    "creation_date": timestamp_to_datetime(
                        instrument.get("creation_timestamp", 0) / 1000
                    ),
                }
    ...
    async def get_option_market_data(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            now = datetime.now(UTC)
            expiry_date = datetime.strptime(option_details["expiry"], "%d%b%y").replace(tzinfo=UTC)

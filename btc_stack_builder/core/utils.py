...
def calculate_annualized_basis(
    futures_price: Decimal, spot_price: Decimal, days_to_expiry: int
) -> Decimal:
    """
...
def calculate_margin_ratio(wallet_balance: Decimal, maintenance_margin: Decimal) -> Decimal:
    """
...
def calculate_option_greeks(
    ...
    if option_type.lower() == "call":
        ...
        theta = -s * norm.pdf(d1) * v / (2 * math.sqrt(t)) + r * k * math.exp(-r * t) * norm.cdf(
            -d2
        )
...
def calculate_position_pnl(
    ...
    return {"absolute_pnl": adjusted_pnl, "percentage_pnl": percentage_pnl, "roi": roi}


def calculate_drawdown(peak_value: Decimal, current_value: Decimal) -> dict[str, Decimal]:
    """
    ...
    """
    if peak_value <= 0:
        return {"absolute_drawdown": Decimal("0.0"), "percentage_drawdown": Decimal("0.0")}
    ...
    return {"absolute_drawdown": absolute_drawdown, "percentage_drawdown": percentage_drawdown}


def calculate_annualized_return(
    initial_value: Decimal, final_value: Decimal, days_held: int
) -> Decimal:
    """
...
def calculate_rolling_cost(
    ...
    return {"basis_cost": basis_cost, "fees_cost": total_fees, "total_cost": total_cost}


def format_btc_amount(amount: Decimal, precision: int = 8, include_symbol: bool = True) -> str:
    """
...
def format_percentage(percentage: Decimal, precision: int = 2, include_symbol: bool = True) -> str:
    """
...
def parse_quarterly_futures_symbol(symbol: str) -> datetime | None:
    """
...
def get_next_quarterly_expiry(current_date: datetime | None = None) -> datetime:
    """

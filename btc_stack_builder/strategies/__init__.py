...
from btc_stack_builder.core.models import (
    ...
    BasisHarvestConfig, FundingCaptureConfig, OptionPremiumConfig
)
from btc_stack_builder.core.constants import (
    BASIS_ENTRY_THRESHOLD, BASIS_MAX_LEVERAGE, BASIS_ROLL_START_DAYS, BASIS_ROLL_END_DAYS,
    FUNDING_ENTRY_THRESHOLD, FUNDING_MAX_LEVERAGE, FUNDING_PROFIT_TARGET,
    OPTION_DELTA_TARGET, OPTION_MIN_EXPIRY_DAYS, OPTION_MAX_EXPIRY_DAYS
)
...
from btc_stack_builder.core.logger import logger, log_strategy_execution
...
class BasisHarvestStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        gateway: ExchangeGateway,
        sub_portfolio: SubPortfolio,
        config: Union[BasisHarvestConfig, Dict[str, Any]]
    ):
        """
        ...
        self.current_quarterly_symbol: Optional[str] = None
        self.next_quarterly_symbol: Optional[str] = None
        ...
        try:
            ...
            if self.current_quarterly_symbol:
                ...
                if expiry_date:
                    ...
                    if days_to_expiry <= BASIS_ROLL_START_DAYS and days_to_expiry > BASIS_ROLL_END_DAYS:
            ...
            if basis >= self.config.entry_threshold:
                ...
                if not self.positions:
                    ...
                    await self.gateway.set_leverage(
                        self.current_quarterly_symbol, 
                        int(self.config.max_leverage)
                    )
    ...
    async def harvest_profits(self) -> Decimal:
        ...
        logger.info(
            "Harvesting Basis profits",
            amount=str(harvested_amount)
        )
...
class FundingCaptureStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        gateway: ExchangeGateway,
        sub_portfolio: SubPortfolio,
        config: Union[FundingCaptureConfig, Dict[str, Any]]
    ):
        """
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Could not retrieve funding history",
                error=str(e)
            )
    ...
    async def harvest_profits(self) -> Decimal:
        ...
        logger.info(
            "Harvesting Funding Capture profits",
            amount=str(harvested_amount)
        )
...
class OptionPremiumStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        gateway: ExchangeGateway,
        sub_portfolio: SubPortfolio,
        config: Union[OptionPremiumConfig, Dict[str, Any]]
    ):
        """
    ...
    async def initialize(self) -> None:
        ...
        self.initialized = True
        logger.info(
            "Option Premium strategy initialized",
        ...
        try:
            ...
            valid_options = [
                ...
                if (
                    ...
                    self.config.min_expiry_days <= (opt["expiry_date"] - now).days <= self.config.max_expiry_days
                )
            ]
            ...
            max_contracts = (available_capital / best_option["strike_price"]).quantize(Decimal("0.01"))
    ...
    async def harvest_profits(self) -> Decimal:
        ...
        logger.info(
            "Harvesting Option Premium profits",
            amount=str(harvested_amount)
        )
    ...
    async def close_all_positions(self) -> bool:
        ...
        for option in self.options:
            ...
            try:
                ...
                option.exit_time = datetime.now(timezone.utc)
        ...
        for days in [60, 75, 90]:
            ...
            for strike_pct in [0.7, 0.75, 0.8, 0.85, 0.9, 0.95]:
                ...
                option_chain.append({
                    ...
                    "strike_price": strike,
                    "expiry_date": expiry,
                    "bid": (spot_price * Decimal("0.01") * (Decimal("1") - strike_pct)).quantize(Decimal("0.0001")),
                    "ask": (spot_price * Decimal("0.012") * (Decimal("1") - strike_pct)).quantize(Decimal("0.0001")),
